...
const PurchaseModal = ({ isOpen, onClose, itemData, updatePurchase }) => {
    ...
    useEffect(() => {
        const fetchItemData = async () => {
            setLoading(true);
            try {
                const newItem = await fetchPurchasesById(itemData.id, value.id);
                setEthanol(newItem);
                setItem(newItem);
                setCount(newItem.count);
                setDate(newItem.date);
                setPrice(newItem.price);
                setDetailName(newItem.detailName);
            } catch (error) {
    ...
    const isBadPurchase = () => {
        return item.count.length === 0 || item.date.length === 0 || item.detailName.length === 0 || item.price.length === 0;
    }
    ...
    const submitButtonClick = () => {
        if (isBadPurchase()) {
            setIsNeedText(true);
            notify();
        } else if (ethanol.price !== item.price || ethanol.date !== item.date || item.detailName !== ethanol.detailName) {
            setIsNeedText(false);
            updatePurchase(item, setLoading);
        }
    }
